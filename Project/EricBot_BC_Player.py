...
class State:
    def __init__(self, old_board=INITIAL, whose_move=WHITE, kingPos=[], frozen=[]):
        ...
        if len(kingPos) == 0:
            self.kingPos = king_search(old_board)
        else:
            self.kingPos = [(k[0], k[1]) for k in kingPos]
        if len(frozen) == 0:
            self.frozen = freezer_search(old_board, whose_move)
        else:
            self.frozen = [[(f[0], f[1]) for f in i] for i in frozen]
        self.eval = None
...
def makeMove(currentState, currentRemark, timelimit=10):
    ...
    remark = ['Nice move, ' + str(OPP_NAME) + '!', 'This is taking too long. Let\'s end this quick.',
              ...
              'Go easy on me, I am new here.', 'Do you want to listen to my music?',
...
def minimax(state, depth, min_or_max, endTime, alphaBeta=True, useZobristHashing=True):
    ...
    if len(s_node.children) == 0:
        ...
        for c_hashcode in s_node.children:
            if ZOBRIST_M[c_hashcode].state.whose_move == (1 - ori_state.whose_move) and ZOBRIST_M[
                c_hashcode].state.board != ori_state.board:
                child_states.append(ZOBRIST_M[c_hashcode].state)
...
def move(state, z_hash, xPos, yPos):
    ...
    for i, j in directions:
        ...
        while x + i >= 0 and y + j >= 0 and x + i <= 7 and y + j <= 7 and (
                state.board[x + i][y + j] == 0 or piece_t == INIT_TO_CODE['k']):
    ...
    if is_on_board(x, y):
        ...
        for i, j in vec[0:4]:
            if is_on_board(x + i, y + j) and is_on_board(x + 2 * i, y + 2 * j) \
                    ...
                    and state.board[x + 2 * i][y + 2 * j] + state.whose_move == INIT_TO_CODE['P'] and (
                    (x - x0, y - y0) in vec[0:4]):
