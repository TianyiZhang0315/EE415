'''Albot_BC_Player.py
This is a Baroque chess agent that coded by Tianyi Zhang and Mingyu Wang.
'''
...
#staic variables
BEST_STATE = None
TIME_LIMIT_OFFSET = 0.2
LAST_MOVE = [-1, -1, -1] # Piece, col, row
CUTOFFS = 0
COUNT = 0
#GAME COUNT
GSESC = 0 #game_static_evals_saved_count = 0
GPC = 0 #game_put_count = 0
GGSC = 0 #game_get_success_count = 0
GGFC = 0 #game_get_failure_count = 0
GCC = 0 #game_collision_count = 0
#MOVE COUNT
MPC = 0 #move_put_count = 0
MGSC = 0 #move_get_success_count = 0
MGFC = 0 #move_get_failure_count = 0
MCC = 0 #move_collision_count = 0
MSESC = 0 #move_static_evals_saved_count = 0
OPP_NAME = None
ZOBRIST = False
ALPHA_BETA = True
N_CHILD = 0
...
def makeMove(currentState, currentRemark, timelimit=10):
    ...
    global BEST_STATE
    global CUTOFFS
    global ZOBRIST, ALPHA_BETA
    print('Zobrist Hashing =', ZOBRIST)
    print('Alpha-Beta pruning =', ALPHA_BETA)
    ...
    resultState = IDDFS(newState, now + timedelta(0, timelimit))
    move = (resultState.move_start_sq, resultState.move_end_sq)
    #show cutoffs
    print('CUTOFFS =', CUTOFFS)
    remark = ['Nice move, ' + str(OPP_NAME) + '!', 'This is taking too long. Let\'s end this quick.',
              'I am not really good at playing games but I will try my best.', \
              'I think I made a decent move by having ' + str(CUTOFFS) + ' cutoffs from alpha-beta pruning.',
              'I think I will win this.', 'Release the Kraken!',
              'I will buy you a coffee if you let me win :)', \
              'Go easy on me, I am new to this game.',
              'I choose this move from ' + str(COUNT) + ' moves.']
    newRemark = random.choice(remark)

    return [[move, resultState], newRemark]

def IDDFS(currentState, endTime):
    """This function is called in makeMove() to perform the search"""
    global ZOBRIST, ALPHA_BETA, N_CHILD
    global MPC, MGSC, MGFC, MCC, MSESC
    global GPC, GGSC, GGFC, GCC, GSESC
    MPC = 0 #move_put_count = 0
    MGSC = 0 #move_get_success_count = 0
    MGFC = 0 #move_get_failure_count = 0
    MCC = 0 #move_collision_count = 0
    ...
    while datetime.now() < endTime-timedelta(0,0.2):
        depth += 1
        best_state = parameterized_minimax(currentState, depth, min_or_max, endTime)
        if best_state != None:
            best = best_state
        else:
            break
    ...
    print('MPC =', MPC,'MGSC =', MGSC,'MGFC =', MGFC,'MSESC =', MSESC)
    print('GPC =', GPC,'GGSC =', GGSC,'GGFC =', GGFC,'GSESC =', GSESC)
    
    return best

def over_time(endTime):
    """This function determine if the it is over time."""
    global TIME_LIMIT_OFFSET
    now = datetime.now()
    return (now + timedelta(0, TIME_LIMIT_OFFSET)) >= endTime #change TIME_LIMIT_OFFSET to avoid overtinme

def minimax_tool(ori_state, state, depth, min_or_max, endTime, alpha, beta, alphaBeta=True, useZobristHashing=True):
    """This function performs actual minimax search."""
    global ZOBRIST_M
    global CUTOFFS 
    global COUNT
    ori_state = ori_state

    if over_time(endTime):
        return None
    ...
    while len(child_states) != 0:
        ...
        new_state = minimax_tool(ori_state, c_state, depth-1, -min_or_max, endTime, alpha, beta, alphaBeta)
        if new_state != None:
...
def parameterized_minimax(state, depth, min_or_max, endTime, alphaBeta=True, useZobristHashing=True):
    """This function prepares for minimax search and output search result"""
    global CUTOFFS, COUNT, N_CHILD
    global MPC, MGSC, MGFC, MCC, MSESC
    global GPC, GGSC, GGFC, GCC, GSESC
    ori_state = state
    ...
    if useZobristHashing:
        s_node = ZOBRIST_M.get(s_hashcode, minimax_tree_node(state))
        if s_node == minimax_tree_node(state):
            MGFC += 1
            GGFC += 1
        else:
            MGSC += 1
    ...
    while len(child_states) != 0:
        ...
        if over_time(endTime): #random choose if overtime
        ...
        new_state = minimax_tool(ori_state, c_state, depth-1, -min_or_max, endTime, alpha, beta, alphaBeta, useZobristHashing)
...
def getHashCode(board):
    """This function is used to get hash code of a board."""
    val = 0
    for x in range(8):
        for y in range(8):
            piece = board[x][y]
            if piece != 0:
                val ^= ZOBRIST_N[8*x+y][piece] #XOR operation
    return val

def getChildStates(state, hashCode):
    """This function get children states of current state."""
    board = state.board
    child_states = []
...
def move(state, z_hash, xPos, yPos):
  """This function returns a list of child state that can move."""
...
def pincer_capture(state, x, y, z_h):
    ...
    for i, j in vec[0:4]:
        if on_board(x, y) and on_board(x+i, y+j) and on_board(x+2*i, y+2*j)\
...
def imitator_capture(state, x, y, x0, y0, i, j, z_h):
    ...
    if on_board(x,y):
        ...
        for i, j in vec[0:4]:
            if on_board(x+i, y+j) and on_board(x+2*i, y+2*j)\
...
def withdrawer_capture(state, x, y, x1, y1, z_h):
    global ZOBRIST_M
    if on_board(x, y) and who(state.board[x][y])!= who(state.board[x1][y1]):
        z_h ^= ZOBRIST_N[8*x+y][state.board[x][y]]
        state.board[x][y] = 0
    state.self_static_eval()
...
def king_capture(state, x, y, x1, y1, z_h):
    global ZOBRIST_M
    if on_board(x,y) and on_board(x1, y1)\
...
def introduce():
  return "I'm Albot, a Baroque Chess agent. I am created by Mingyu Wang and Tianyi Zhang. UW NETID myw518 and zhang506."
...
PIECE_VALS = [0,0,-1,1,-2,2,-5,5,-7,7,-8,8,-100,100,-3,3]#eval values for each piece
