...
STUDENTS: COMPLETE THE IMPLEMENTATION OF AStar.py BY WRITING CODE WHERE
...
print("\nWelcome to AStar, by Tianyi Zhang!")
...
# STEP 1b. Assign g=0 to the start state.
  g[initial_state]=0.0
  f[initial_state] = g[initial_state]+Problem.h(initial_state)
...
#         and if it is already on CLOSED, delete the new instance.
    if 'France-Trip' in Problem.PROBLEM_NAME:# get name of problem to determine what method to use
      i = 0
        ...
        if not S.ith_neighbor_exists(i):
          for j in range(i):
            ...
            if ne not in CLOSED and ne not in OPEN: #make sure neighbor not in CLOSED
              ...
              f[ne] = g[ne] + hd
              OPEN.insert(ne, f[ne])
              print(OPEN,'G:',g[ne],'H:',hd)
            elif ne in CLOSED:#if some state in CLOSED can be updated with smaller F value, put back to OPEN with new F value
              g[ne] = g[S] + P
              f[ne] = g[ne] + hd
              if f[ne] < CLOSED[ne]:
                OPEN.insert(ne, f[ne])
          break
        i+=1
    elif 'Eight Puzzle' in Problem.PROBLEM_NAME:
      for dir in Problem.directions:
        if S.can_move(dir):
          new_S = S.move(dir)
          P = 1
          hd = Problem.h(new_S)
          if new_S not in CLOSED and new_S not in OPEN:#insert (State,f_value) into OPEN
            BACKLINKS[new_S] = S
            g[new_S] = g[S] + P
            f[new_S] = g[new_S] + hd
            OPEN.insert(new_S, f[new_S])
            #print(OPEN, 'G:', g[new_S], 'H:', hd)
          elif new_S in CLOSED:#if some state in CLOSED can be updated with smaller F value, put back to OPEN with new F value
            g[new_S] = g[S] + P
            f[new_S] = g[new_S] + hd
            if f[new_S] < CLOSED[new_S]:
              OPEN.insert(new_S, f[new_S])
